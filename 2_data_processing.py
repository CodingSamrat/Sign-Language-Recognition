...
def CreateColumns(_imgSize):
    ...
    print(f"{featuresClmLength} Columns are Created Succeaafully!\n")
    return columns

def Image2CSV(_path, _columns, _imgSize):
    """
    It takes three parameters Path & List of Columns and img Size.
    ...
    for dir in os.listdir(_path):
        for img in tqdm(os.listdir(f"{_path}/{dir}")):
            ...
            features.append(img)                            # Appending Features
    ...
    df = pd.DataFrame(features, columns=_columns)            # Converting Features(np array) into a Pandas DataFrame
    df.to_csv("features.csv", index=False)                  # Saving Features DataFrame into CSV File

    df = pd.DataFrame(labels, columns=["label"])                               # Converting Labels(np array) into a Pandas DataFrame
    df.to_csv("labels.csv", index=False)                    # Saving Labels DataFrame into CSV File
